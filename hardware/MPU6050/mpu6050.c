...
{
	MyIIC_Write_MultiBytes(addr, reg, len, buf);
	return 0;
}
// IIC连续读
// addr:器件地址
...
{
	MyIIC_Read_MultiBytes(addr, reg, len, buf);
	return 0;
}
// IIC写一个字节
// reg:寄存器地址
...
{
	MyIIC_Write_SingleByte(MPU_ADDR, reg, data);
	return 0;
}
// IIC读一个字节
// reg:寄存器地址
...
{
	u8 res;
	res = MyIIC_Read_SingleByte(MPU_ADDR, reg);
	return res;
}
